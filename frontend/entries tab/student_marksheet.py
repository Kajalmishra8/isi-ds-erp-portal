#frontend>entries_tab>student_marksheet.py

import streamlit as st
import pandas as pd
from utils.api_client import get
from components.ui import page_header, section_label, empty_state, badge, info_banner

def _grade_color(grade: str) -> str:
    return {"A+": "green", "A": "green", "B": "blue", "C": "yellow", "F": "red"}.get(grade, "gray")
...
def show():
    ...
    if not exams:
        empty_state(
            "No exams available",
            "There are no active exams for your semester right now. Check back later.",
            "📋",
        )
        return

    exam_map = {f"{e['exam_name']} ({e.get('year','')})": e["exam_id"] for e in exams}

    with st.form("marksheet_form"):
        section_label("Select Examination")
        selected = st.selectbox("Exam", list(exam_map.keys()), label_visibility="visible")
    ...
    if submitted:
        ...
        with st.spinner("Fetching your marksheet…"):
            data = get(
                f"/api/student/marksheet?enroll_no={enroll_no}&exam_id={exam_map[selected]}"
            )
          ...
          <div style="display:flex;justify-content:space-between;align-items:flex-start;">
            <div>
              <div style="font-size:18px;font-weight:700;color:#111827;
                          letter-spacing:-.01em;">{student.get('name','')}</div>
        ...
        col1.metric("Total Obtained", summary.get("total_obtained", 0))
        col2.metric("Maximum Marks",  summary.get("total_max", 0))
        col3.metric("Percentage",     f"{summary.get('percentage', 0)}%")
        col4.metric("Grade",          summary.get("grade", "—"))
        ...
        if marks:
            ...
            if "Obtained" in df.columns and "Max" in df.columns:
                df["Score %"] = ((df["Obtained"] / df["Max"]) * 100).round(1)
            st.dataframe(
                df,
        ...
        if marks:
            ...
            st.download_button(
                ...
                csv,
                file_name=f"marksheet_{enroll_no}_{exam.get('name','')}.csv",
                mime="text/csv",
                use_container_width=True,
            )